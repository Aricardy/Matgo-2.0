...
export default function BookingPage() {
  ...
  const [backRowSeats, setBackRowSeats] = useState<string[]>([]);
  ...
  useEffect(() => {
    // Fetch route and configuration data from backend
    const fetchData = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            setLongDistanceRoutes(fallbackRoutes);
          }
          
          // Set default time slots and seat configuration
          setTimeSlots(["06:00", "08:00", "10:00", "12:00", "14:00", "16:00", "18:00"]);
          setSeatRows(12);
          setSeatsPerRow(['A', 'B', 'C', 'D']);
          setBackRowSeats(['E1', 'E2', 'E3', 'E4', 'E5']);
      ...
      const fetchSeats = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            // Transform backend seat data to expected format
            const transformedSeats = seatData.map((seat: any) => ({
              id: seat.seatId || seat.id,
              isBooked: seat.isBooked || seat.status === 'booked',
              isSelected: false,
              passengerName: ""
            }));
  ...
  const content = {
    ENG: {
      ...
      noRoutesAvailable: "No long distance routes are available at the moment. Please check back later."
    },
    KSW: {
      ...
      noRoutesAvailable: "Hakuna safari za mbali zinazopatikana kwa sasa. Tafadhali angalia tena baadaye."
    }
  };
  ...
  const renderSeats = () => {
    if (isLoading || seats.length === 0) return <div className="text-center py-8 text-muted-foreground">{language === 'KSW' ? 'Inapakia viti...' : 'Loading seats...'}</div>
    
    // Hardcoding layout for this simulation
    ...
    if (step === 1 && selectedRouteValue && selectedDate && selectedTime && numberOfSeatsToBook > 0) {
      ...
      setStep(3);
        ...
        const bookingData = {
          ...
          passengers: selectedSeatsList.map(id => ({
            seatId: id,
            name: seats.find(s => s.id === id)?.passengerName || (language === 'KSW' ? "Mwenyewe" : "Self")
          })),
    ...
    <AppLayout>
      <div className="space-y-8 animate-fade-in">
        ...
        <Card className="shadow-xl glassy-card overflow-hidden">
          <CardHeader className={cn("p-6", currentRouteDetails?.image && "relative")}>
            {currentRouteDetails?.image && (
              <>
                <Image src={currentRouteDetails.image} alt={currentRouteDetails.label} data-ai-hint={currentRouteDetails.imageHint} width={800} height={250} className="absolute inset-0 w-full h-full object-cover"/>
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent"></div>
              </>
            )}
            <div className={cn("relative z-10", currentRouteDetails?.image && "text-white")}>
            ...
            <CardContent className="space-y-6 pt-4">
              {longDistanceRoutes.length > 0 ? (
                ...
                <Alert variant="default" className="border-yellow-500 text-yellow-700 dark:border-yellow-400 dark:text-yellow-300 bg-yellow-500/10">
                    ...
                    <AlertTitle>{currentContent.noRoutesAvailable}</AlertTitle>
                </Alert>
              )}
            </CardContent>
          </Card>
