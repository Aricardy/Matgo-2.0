...
interface SaccoMessage { id: string; message: string; timestamp: string; sender: string; isFromAdmin: boolean; }
...
export default function CrewDashboardPage() {
  ...
  const [saccoMessages, setSaccoMessages] = useState<SaccoMessage[]>([]);
        ...
        if (storedUser) {
            try {
                ...
                if (userData.role === 'driver' || userData.role === 'conductor') {
                    ...
                    // Fetch crew-specific data from backend
                    try {
                        ...
                        if (response.ok) {
                            ...
                            setCrewMember({
                                name: crewData.name || `${userData.firstName || 'John'} ${userData.lastName || 'Doe'}`,
                                role: language === 'KSW' ? (userData.role === 'driver' ? 'Dereva' : 'Kondakta') : (userData.role === 'driver' ? 'Driver' : 'Conductor'),
                                sacco: crewData.sacco || userData.saccoName || "MatGo Fleet",
                                busName: crewData.busName || crewData.vehicleName || "SM001 - Mamba",
                                busReg: crewData.busReg || crewData.plateNumber || "KDA 001A",
                                profilePic: crewData.profilePic || crewData.avatar || "/avatars/crew_default.png",
                                rating: crewData.rating || 4.7
                            });
                            
                            // Set duty status from backend if available
                            if (crewData.isOnDuty !== undefined) {
                                setIsOnDuty(crewData.isOnDuty);
                            }
                            
                            // Set current route if available
                            if (crewData.currentRoute) {
                                setCurrentRoute(crewData.currentRoute);
                            }
                        } else {
                        ...
                        // Fetch trip logs
                        const tripLogResponse = await fetch('/api/trips', {
                            headers: {
                                'Authorization': token ? `Bearer ${token}` : ''
                            }
                        });
                        
                        if (tripLogResponse.ok) {
                            const trips = await tripLogResponse.json();
                            // Filter trips for this crew member and format for display
                            const userTrips = trips.filter((trip: any) => 
                                trip.driverId === userData.id || trip.conductorId === userData.id
                            ).map((trip: any) => ({
                                id: trip.id,
                                ...
                                fareCollected: trip.fareCollected || trip.totalPrice || 0
                            }));
  ...
  const handleDutyToggle = async (checked: boolean) => {
    ...
    try {
      if (storedUser) {
        ...
        const response = await fetch(`${endpoint}/${userData.id}/duty-status`, {
          method: 'PUT',
          headers: {
            'Authorization': token ? `Bearer ${token}` : '',
        ...
        if (response.ok) {
          setIsOnDuty(checked);
          toast({ 
            title: currentText.dutyStatusChanged, 
            description: currentText.dutyStatusChangedDesc(checked ? (language === 'KSW' ? 'Kazini' : 'On Duty') : (language === 'KSW' ? 'Nje ya Kazi' : 'Off Duty')) 
          });
      ...
      if (storedUser) {
        ...
        if (!isTripActive) {
          ...
          const response = await fetch('/api/trips/start', {
            ...
            body: JSON.stringify({
              route: currentRoute,
              driverId: userData.role === 'driver' ? userData.id : null,
              conductorId: userData.role === 'conductor' ? userData.id : null,
              vehicleId: crewMember?.busReg,
          ...
          if (response.ok) {
            ...
            setPassengerCount(0);
            toast({ 
              title: currentText.tripStartedToast, 
              description: currentText.tripStartedToastDesc(currentRoute), 
          ...
          const response = await fetch('/api/trips/end', {
            ...
            body: JSON.stringify({
              ...
              fareCollected: passengerCount * 100 // Estimate
            })
          });
          ...
          if (response.ok) {
            ...
            setPassengerCount(0);
            toast({ 
              title: currentText.tripStoppedToast, 
              description: currentText.tripStoppedToastDesc 
            });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (storedUser) {
        ...
        const response = await fetch('/api/messages/sacco', {
          method: 'POST',
          headers: {
            'Authorization': token ? `Bearer ${token}` : '',
            'Content-Type': 'application/json'
          ...
          body: JSON.stringify({
            ...
            senderName: crewMember?.name || userData.firstName,
            timestamp: new Date().toISOString()
          })
        });
        ...
        if (response.ok) {
          ...
          setSaccoMessages(prev => [...prev, {
            id: sentMessage.id || Date.now().toString(),
            message: newMessage,
            timestamp: new Date().toISOString(),
            sender: crewMember?.name || userData.firstName || 'You',
            isFromAdmin: false
          }]);
          setNewMessage("");
          toast({ 
            title: language === 'KSW' ? 'Ujumbe Umetumwa' : 'Message Sent', 
            description: language === 'KSW' ? 'Ujumbe wako umetumwa kwa SACCO.' : 'Your message has been sent to SACCO.' 
          });
        } else {
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <Tabs defaultValue="dashboard" className="w-full">
          ...
          <TabsContent value="tripLog">
            <Card className="shadow-lg glassy-card">
              <CardHeader><CardTitle className="font-headline text-2xl text-accent">{currentText.tripLogTitle}</CardTitle></CardHeader>
              <CardContent>
                {tripLog.length > 0 ? (
                  <div className="space-y-4">
                    {tripLog.map((trip) => (
                      <div key={trip.id} className="p-4 border border-border/50 rounded-lg bg-card/50">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="text-right">
                            <p className="text-sm">{trip.startTime} - {trip.endTime}</p>
                            <p className="text-sm text-muted-foreground">{trip.passengers} passengers</p>
                          </div>
                        </div>
                        <div className="mt-2 pt-2 border-t border-border/30">
                          <p className="text-sm font-medium">Fare Collected: Ksh {trip.fareCollected.toLocaleString()}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-muted-foreground text-center py-8">{currentText.noTripsLogged}</p>
                )}
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="saccoComms">
             <Card className="shadow-lg glassy-card">
                ...
                <CardContent className="space-y-4">
                    <div className="h-48 border rounded-md p-3 bg-muted/20 overflow-y-auto text-sm">
                      {saccoMessages.length > 0 ? (
                        <div className="space-y-3">
                          {saccoMessages.map((msg) => (
                            <div key={msg.id} className={`p-2 rounded-lg ${msg.isFromAdmin ? 'bg-primary/10 border-l-4 border-primary' : 'bg-accent/10 border-l-4 border-accent ml-4'}`}>
                              <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center justify-center h-full">
                          <p className="text-muted-foreground">{currentText.noMessages}</p>
                        </div>
                      )}
                    </div>
                    <div className="flex gap-2">
                      <Input 
                        placeholder={currentText.typeMessagePlaceholder} 
                        value={newMessage}
                        onChange={(e) => setNewMessage(e.target.value)}
                        onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
                        className="flex-1 rounded-lg text-base py-2.5"
                      />
                      <Button 
                        onClick={handleSendMessage}
                        disabled={!newMessage.trim()}
                        className="btn-glow-primary rounded-lg text-base py-2.5"
                      >
                        {currentText.sendMessage}
                      </Button>
                    </div>
                </CardContent>
             </Card>
